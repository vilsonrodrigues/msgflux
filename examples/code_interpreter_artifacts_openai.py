...
Use await artifacts["info"](name) to inspect metadata.
Use await artifacts["read"](name, offset=0, limit=...) to read bounded slices.
...
def build_agent(model_name: str) -> nn.Agent:
    return nn.Agent(
        ...
        instructions=(
            ...
            "Inside Python, await artifacts[\"info\"]('runtime_notes') returns a "
            "dict, so read the size with info['size']. artifacts[\"read\"] "
            "requires keyword arguments: await artifacts[\"read\"]('runtime_notes', "
            "offset=0, limit=800), and returns a str. The interpreter only returns "
            "stdout and the result variable; bare expressions are not returned. "
            "For debug, prefer print(...) over storing large text in variables. "
            "Pass the bounded artifact slice directly into "
            'await tools["llm_query"](..., context=await artifacts["read"](...)) '
...
async def run_with_artifact(model_name: str, artifact_path: Path) -> None:
    ...
    response = await agent.acall(
        (
            ...
            "print(await artifacts['read']('runtime_notes', offset=0, limit=300))\n"
            "answer = await tools['llm_query'](\n"
            "    task='How do session_id, run_id, namespace and artifacts relate "
            "to durable execution?',\n"
            "    context=await artifacts['read'](\n"
            "        'runtime_notes', offset=0, limit=800\n"
            "    ),\n"
            ")\n"
