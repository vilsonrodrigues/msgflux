"""ChatMessages — container for chat history with multi-format support.
...
- Multi format output (ChatML, Responses API, Google GenAI)
...
# Dummy thought_signature for Gemini function calls not generated by Gemini.
# Required when migrating conversations from other models or injecting custom
# function calls. See: https://ai.google.dev/gemini-api/docs
_GENAI_DUMMY_THOUGHT_SIGNATURE = "context_engineering_is_the_way to_go"
...
class ChatMessages:
    ...
    def to_gemini(self) -> Tuple[Optional[str], List[dict[str, Any]]]:  # noqa: C901
        ...
        - system_instruction: Combined system/developer messages (or None)
        - contents: List of Content dicts with role='user' or role='model'

        Google GenAI uses a different structure:
        - Roles: 'user' and 'model' (no 'assistant' or 'system')
        - System instructions are a separate parameter, not in contents
        - Parts use 'text', 'inline_data', 'function_call', 'function_response'
        """
        system_parts: List[str] = []
        contents: List[dict[str, Any]] = []

        # Track pending function calls to map call_id -> function name
        # (needed because function_response requires the function name)
        ...
        for item in self._items:
            ...
            if item_type == "function_call":
                ...
                args = self._genai_parse_arguments(arguments)
                part: dict[str, Any] = {"function_call": {"name": name, "args": args}}
                if call_id:
                    part["function_call"]["id"] = call_id
                part["thought_signature"] = item.get(
                    "thought_signature", _GENAI_DUMMY_THOUGHT_SIGNATURE
                )
                self._genai_append_part(contents, "model", part)
                continue

            if item_type == "function_call_output":
                call_id = item.get("call_id")
                output = item.get("output")
                name = call_id_to_name.get(call_id, "") if call_id else ""
                if not isinstance(output, dict):
                    output = {"result": str(output) if output is not None else ""}
                part = {"function_response": {"name": name, "response": output}}
                if call_id:
                    part["function_response"]["id"] = call_id
                self._genai_append_part(contents, "user", part)
                continue

            if item_type == "message":
                role = item.get("role")
                if role in ("system", "developer"):
                    text = self._genai_extract_text(item.get("content"))
                    if text:
                        system_parts.append(text)
                    continue
                genai_role = "model" if role == "assistant" else "user"
                parts = self._genai_convert_content(item.get("content"))
                if parts:
                    self._genai_extend_parts(contents, genai_role, parts)
                continue
            ...
            if role in ("system", "developer"):
                content = item.get("content")
                text = self._genai_extract_text(content)
                if text:
                    system_parts.append(text)
                continue
            ...
            if role == "assistant":
                ...
                if tool_calls:
                    for call in tool_calls:
                        ...
                        args = self._genai_parse_arguments(function.get("arguments"))
                        fc_part: dict[str, Any] = {
                            "function_call": {"name": fname, "args": args}
                        }
                        if cid:
                            fc_part["function_call"]["id"] = cid
                        fc_part["thought_signature"] = call.get(
                            "thought_signature", _GENAI_DUMMY_THOUGHT_SIGNATURE
                        )
                        self._genai_append_part(contents, "model", fc_part)

                content = item.get("content")
                if content not in (None, "", []):
                    parts = self._genai_convert_content(content)
                    if parts:
                        self._genai_extend_parts(contents, "model", parts)
                elif not tool_calls:
                    parts = self._genai_convert_content(content)
                    if parts:
                        self._genai_extend_parts(contents, "model", parts)
            ...
            if role == "tool":
                ...
                fr_part: dict[str, Any] = {
                    "function_response": {"name": name, "response": output}
                }
                if call_id:
                    fr_part["function_response"]["id"] = call_id
                self._genai_append_part(contents, "user", fr_part)
                continue

            if role == "user":
                parts = self._genai_convert_content(item.get("content"))
                if parts:
                    self._genai_extend_parts(contents, "user", parts)
                continue
    ...
    @staticmethod
    def _genai_append_part(
        contents: List[dict[str, Any]], role: str, part: dict[str, Any]
    ) -> None:
        """Append a single part, merging into the last content if same role."""
    ...
    @staticmethod
    def _genai_extend_parts(
        contents: List[dict[str, Any]], role: str, parts: List[dict[str, Any]]
    ) -> None:
        """Extend with multiple parts, merging into the last content if same role."""
        if contents and contents[-1].get("role") == role:
    ...
    @staticmethod
    def _genai_parse_arguments(arguments: Any) -> dict[str, Any]:
        """Parse function arguments to a dict."""
    ...
    @staticmethod
    def _genai_extract_text(content: Any) -> str:
        """Extract plain text from content (str or list of parts)."""
    ...
    def _genai_convert_content(self, content: Any) -> List[dict[str, Any]]:  # noqa: C901
        """Convert ChatML content to Google GenAI parts."""
        ...
        for item in content:
            ...
            if part_type == "image_url":
                ...
                if isinstance(image_url, Mapping):
                    ...
                    genai_part = self._genai_convert_media_url(url, "image")
                    if genai_part:
                        parts.append(genai_part)
                continue
            ...
            if part_type == "input_audio":
                ...
                if isinstance(input_audio, Mapping):
                    ...
                    fmt = input_audio.get("format", "mp3")
                    mime = f"audio/{fmt}"
                    raw = base64.b64decode(data) if isinstance(data, str) else data
                    parts.append({"inline_data": {"data": raw, "mime_type": mime}})
                continue

            # Audio (audio_url format with data URI)
            if part_type == "audio_url":
                audio_url = item.get("audio_url", {})
                if isinstance(audio_url, Mapping):
                    url = audio_url.get("url", "")
                    genai_part = self._genai_convert_media_url(url, "audio")
                    if genai_part:
                        parts.append(genai_part)
                continue
            ...
            if part_type == "video_url":
                ...
                if isinstance(video_url, Mapping):
                    url = video_url.get("url", "")
                    genai_part = self._genai_convert_media_url(url, "video")
                    if genai_part:
                        parts.append(genai_part)
                continue
            ...
            if part_type == "file":
                ...
                if isinstance(file_item, Mapping):
                    ...
                    mime = self._genai_guess_mime(filename)
                    if file_data:
                        genai_part = self._genai_convert_media_url(file_data, "file")
                        if genai_part:
                            parts.append(genai_part)
                    elif file_url:
                        parts.append(
                            {"file_data": {"file_uri": file_url, "mime_type": mime}}
                        )
                continue
            ...
            if text is not None:
                ...
                parts.append({"text": msgspec_dumps(dict(item))})

        return parts if parts else [{"text": ""}]

    @staticmethod
    def _genai_convert_media_url(url: str, media_hint: str) -> Optional[dict[str, Any]]:
        """Convert a data URI or URL to a GenAI inline_data or file_data part."""
        if not isinstance(url, str) or not url:
            return None

        marker = ";base64,"
        if url.startswith("data:") and marker in url:
            header, b64_data = url.split(marker, 1)
        ...
        if url.startswith(("http://", "https://", "gs://")):
            ...
            return {
                "file_data": {
                    "file_uri": url,
                    "mime_type": mime_map.get(media_hint, "application/octet-stream"),
                }
            }
    ...
    @staticmethod
    def _genai_guess_mime(filename: str) -> str:
