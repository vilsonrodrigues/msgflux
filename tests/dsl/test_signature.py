...
def test_signature_examples_dataclass():
    ...
    examples = SignatureExamples(
        inputs={"name": "Alice", "age": 25}, outputs={"result": "success"}
    )
...
def test_dtype_to_str_with_union():
    ...
    assert (
        "Union" in inputs[0].dtype or "|" in inputs[0].dtype or "str" in inputs[0].dtype
    )
...
def test_signature_factory_with_audio_types():
    ...
    inputs_info = [FieldInfo(name="sound", dtype="Audio", desc="An audio file")]
...
def test_signature_get_instructions_empty():
    ...
    assert (
        instructions == "" or instructions is None or "NoDocSignature" in instructions
    )
